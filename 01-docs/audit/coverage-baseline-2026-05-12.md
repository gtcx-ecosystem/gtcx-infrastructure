...
| ------------------------------------------------- | ------ | --------------------------------------------------- |
| `03-platform/src/verifier.mjs`                    | 94.59% | Edge case: invalid envelope structure (lines 57–58) |
| `03-platform/src/runtime-policy.mjs`              | 87.50% | Missing `isProductionMode=false` path               |
| `03-platform/src/server.mjs`                      | 58.06% | Error handlers, graceful shutdown, DB failure paths |
| `03-platform/src/protocols-crypto/did-verify.mjs` | 35.71% | DID resolution failure, cache miss, format errors   |
| `03-platform/src/protocols-crypto/jwt-verify.mjs` | 40.00% | Token format edge cases, key fetch failure          |
| `03-platform/src/store/redis-nonce-store.mjs`     | 100%   | Only 21.91% line coverage — requires Redis          |
| `03-platform/src/audit/audit-capture.mjs`         | 71.42% | Audit DB write failure paths                        |
| `03-platform/src/metrics/replay-metrics.mjs`      | 58.33% | Prometheus push failure paths                       |
