...
1. Consequential protocol mutations are reachable from an unauthenticated natural-language HTTP endpoint in `03-platform/tools/compliance-gateway/03-platform/src/server.mjs:43` and `03-platform/tools/compliance-gateway/03-platform/src/tools.mjs:17`.
...
3. Replay protection silently weakens to process-local state when Redis is absent or down in `03-platform/tools/replay-protection/03-platform/src/server.mjs:54`.
...
- `03-platform/tools/replay-protection/03-platform/src/redis.d.ts` no longer carries explicit `any` boundaries (F-007 closed in Sprint 1).
...
- New `03-platform/tools/03-platform/scripts/docs-standard-validator.mjs` replaces the stub `docs-links` job.
...
- New `03-platform/tools/03-platform/scripts/validate-score-ledger.mjs` parses audit docs and fails if any dimension claims a score higher than the ledger's current authorized value.
...
| ------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------- |
| Routing / cost optimization     | Level 2       | Multi-provider routing and fallback exist in `03-platform/tools/compliance-gateway/03-platform/src/server.mjs` | Keep routing, but subordinate it to policy and identity           | Provider choice and fallback should be invisible to operators       |
