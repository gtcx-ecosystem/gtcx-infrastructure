...
**Framework Reference:** `01-docs/05-audit/audit-grade-framework.md` §2.3  
...
cat 01-docs/05-audit/prompts/evidence/enterprise-grade-evidence.json 2>/dev/null | jq '.final_score' || echo "Run E-GAP first"
# Verify Enterprise-Grade ≥ 8.5
cat 01-docs/05-audit/prompts/evidence/enterprise-grade-evidence.json 2>/dev/null | jq '.final_score >= 8.5' || echo "false"
```
...
cat 01-docs/financial/audited-financial-statements-*.pdf 2>/dev/null | head -c 100 || echo "No audited financials found"
# Check for CPA engagement letter
find 01-docs/financial/ -name "*engagement*" -o -name "*letter*cpa*" | head -5
# Check for bookkeeping system
grep -ri "quickbooks\|xero\|sage\|netsuite" 01-docs/financial/ || true
```
...
grep -ri "carta\|pulley\|angelist\|equityzen" 01-docs/financial/ 01-docs/11-legal/ || true
# Check for 2FA enforcement documentation
grep -ri "2fa\|mfa\|two.factor" 01-docs/11-legal/cap-table* || true
# Check for shareholder agreement
cat 01-docs/11-legal/shareholder-agreement.md 2>/dev/null | head -30 || echo "No shareholder agreement found"
```
...
grep -r "transfer\|withdraw\|sign\|private.key\|seed" 03-platform/tools/gtcx-platforms/03-platform/src/ 2>/dev/null | head -10
grep -r "transfer\|withdraw\|sign\|private.key\|seed" 03-platform/tools/gtcx-protocols/03-platform/src/ 2>/dev/null | head -10
# Check for secrets in code
grep -ri "password\|secret\|key.*=.*['\"]" 03-platform/tools/*/03-platform/src/ | grep -v "example\|test\|mock" | head -10
# Check SAST in CI
...
grep -ri "copyright\|proprietary\|confidential" 03-platform/tools/*/03-platform/src/ | grep -v "gtcx" | head -10
...
cat 01-docs/financial/accounting-policies.md 2>/dev/null | head -30 || echo "No accounting policies found"
# Check for related-party transaction policy
grep -ri "related.party\|conflict.*interest" 01-docs/financial/ 01-docs/11-legal/ | head -10
# Check for expense policy
cat 01-docs/financial/expense-policy.md 2>/dev/null | head -20 || echo "No expense policy found"
```
...
grep -ri "bus.factor\|key.person\|succession" 01-docs/ | head -10
# Check documentation coverage
find 03-platform/tools/ -name "*.md" | wc -l
find 03-platform/tools/ -name "README*" | wc -l
# Check runbooks
find 01-docs/devops/ -name "*runbook*" | wc -l
```
...
cat 01-docs/10-compliance/regulatory-contacts.md 2>/dev/null | head -30 || echo "No regulatory contacts found"
# Check for enforcement actions
grep -ri "enforcement\|fine\|penalty\|cease" 01-docs/10-compliance/ 01-docs/11-legal/ | head -10
# Check for license status
grep -ri "license\|registered\|authorized" 01-docs/10-compliance/ | head -10
```
...
cat 01-docs/financial/revenue-analytics.md 2>/dev/null | head -30 || echo "No revenue analytics found"
# Check for customer concentration policy
grep -ri "concentration\|diversification\|single.customer" 01-docs/financial/ | head -10
```
...
cat 01-docs/esg/carbon-footprint.md 2>/dev/null | head -20 || echo "No carbon footprint data"
# Check diversity metrics
grep -ri "diversity\|inclusion\|gender\|demographics" 01-docs/hr/ 01-docs/esg/ | head -10
# Check governance board
cat 01-docs/governance/board-charter.md 2>/dev/null | head -30 || echo "No board charter found"
```
...
Produce a JSON file at `01-docs/05-audit/prompts/evidence/investment-grade-evidence.json`:
