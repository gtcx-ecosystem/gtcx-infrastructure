...
**[P1] Audit Verification Gap — Migration script asserts audit constraints without checking them** `04-ship/03-platform/scripts/migrate.sh:228`  
...
| Infrastructure security        | 7.8/10 | Kyverno policies, non-root security context requirements, and signed-image policy are in place (`04-ship/kubernetes/base/policies/require-security-context.yaml:1`).    |
...
**Deliverables:** Add live verification queries or negative privilege tests to `migrate.sh` and/or CI for the audit DB (`04-ship/03-platform/scripts/migrate.sh:228`).  
