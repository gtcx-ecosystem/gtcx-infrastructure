...
| P1-002 | New audit endpoints lack the same QPS/budget gate used by `/v1/query`. | `03-platform/tools/compliance-gateway/03-platform/src/server.mjs:220-244` gates `/v1/query`; `/audit/bundles` and `/audit/query` handlers at `:560` and `:589` do not call `checkBudget`. | Authenticated principal can flood audit ingestion/query paths.              |
...
| P2-004 | Redis nonce-store coverage remains low inside compliance-gateway. | `03-platform/tools/compliance-gateway/03-platform/src/nonce-store/redis.mjs`: 52.88% statements, 50% branches.                                 | Redis failover and error-handling behavior are under-tested.                                 |
