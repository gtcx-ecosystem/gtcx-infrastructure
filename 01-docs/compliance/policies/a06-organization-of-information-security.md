...
1. **Separation of duties.** No single individual may control all phases of a critical process. The separation of duties matrix (`01-docs/10-compliance/separation-of-duties-matrix.md`) governs access to production systems, code deployment, financial transactions, and audit logs. At minimum: code author cannot approve their own PR; deployer cannot modify production secrets; DBA cannot alter audit database records.
