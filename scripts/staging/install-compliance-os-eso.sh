...
OVERLAY="${REPO_ROOT}/infra/kubernetes/overlays/staging/compliance-os"
...
kubectl get sa compliance-os-sa -n compliance-os-staging -o jsonpath='{.metadata.annotations.eks\.amazonaws\.com/role-arn}{"\n"}' \
  ...
  echo "Run: terraform -chdir=infra/terraform/environments/staging apply -target=module.secrets"
}
...
echo "  docs/operations/coordination/to-compliance-os-hub-17-staging-blockers-witness-2026-06-05.md"
kubectl get externalsecrets -n compliance-os-staging
exit 1
