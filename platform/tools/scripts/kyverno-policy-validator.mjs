...
const POLICIES_DIR = path.join(process.cwd(), 'deploy', 'kubernetes', 'base', 'policies');
const KUSTOMIZATION_FILE = path.join(POLICIES_DIR, 'kustomization.yaml');
const SERVICES_DIR = path.join(process.cwd(), 'deploy', 'kubernetes', 'base', 'services');
const OVERLAYS_DIR = path.join(process.cwd(), 'deploy', 'kubernetes', 'overlays');
