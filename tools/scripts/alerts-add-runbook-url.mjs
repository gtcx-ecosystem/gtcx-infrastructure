#!/usr/bin/env node
/**
 * @fileoverview Ensure every Prometheus alert in infra/monitoring/alerts/
 * carries a `runbook_url` annotation pointing at the canonical alerts
 * runbook section (docs/operations/runbooks/alerts.md#<alertname-lower>).
 *
...
function main() {
  ...
  if (checkOnly) {
    if (allMissing.length > 0) {
      ...
      console.error(
        '\nRun `node tools/scripts/alerts-add-runbook-url.mjs` to add them.'
      );
      process.exit(1);
    }
