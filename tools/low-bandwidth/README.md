...
| Module     | File                 | Purpose                                                           |
| ---------- | -------------------- | ----------------------------------------------------------------- |
| Encoder    | `src/encoder.mjs`    | `json`, `compact-json`, and `minimal-binary` serialization        |
| Negotiator | `src/negotiator.mjs` | Level resolution, encoding selection, replay-window mapping       |
| Trimmer    | `src/trimmer.mjs`    | Schema-based field omission and endpoint-aware response stripping |
| Telemetry  | `src/telemetry.mjs`  | Structured degradation events, alerting, and Prometheus metrics   |
| Middleware | `src/middleware.mjs` | `createTransform` for adaptive HTTP middleware                    |
...
The compliance-gateway (`tools/compliance-gateway`) integrates this toolkit directly via `detectLowBandwidth`, `stripForLowBandwidth`, and `sendJson` in `src/server.mjs`.
