...
| AC-04 | AC-7 Failed Logon Limits | CC6.1 | A.9.4     | Limits on invalid access attempts | Rate limiting via `packages/domain/rate-limit.ts`. **GAP**: No account lockout for DID-based auth. No failed attempt counter.                                                                                                 | PARTIAL     |
...
| ID    | NIST 800-53                       | SOC 2 | ISO 27001 | Description                 | GTCX Implementation                                                                                                                                                                                               | Status      |
| ----- | --------------------------------- | ----- | --------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| SC-01 | SC-8 Transmission Confidentiality | CC6.1 | A.13.1    | Protect transmitted data    | `@gtcx/crypto` provides AES-256-GCM with 12-byte random nonces and authentication tags. PANX envelopes support `encrypted: boolean`. **GAP**: No TLS/mTLS for transport-layer encryption. Application-layer only. | PARTIAL     |
...
| SC-05 | SC-28 Protection at Rest          | CC6.1 | A.8.24    | Encryption of stored data   | AES-256-GCM available via `@gtcx/crypto`. **GAP**: No automatic encryption of stored data. Identity store, audit logs, custody records stored unencrypted. No disk encryption requirements documented.            | GAP         |
...
| SI-02 | SI-10 Input Validation  | CC7.2 | A.14.2    | Input validation            | `@gtcx/validators`: `assertNonEmptyString()`, `assertFiniteNumber()`, `assertValidDid()`, `assertValidJurisdiction()`, `assertFutureTimestamp()`. `@gtcx/schemas`: domain-specific schema validation via `validateById()`. TypeScript `strict: true` with `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`. | IMPLEMENTED |
