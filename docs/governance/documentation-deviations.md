...
| [`../agile/`](../agile/)   | Sprint planning artifacts (20 files). **Canonical execution plan lives at `docs/audit/execution-roadmap.md`** — the files under `docs/agile/` are historical sprint snapshots, templates, or backlog metadata. | Could fold into `operations/sprint-plans/`. Kept top-level for now because operators routinely navigate to `docs/agile/` directly. May fold in a future cleanup. |
