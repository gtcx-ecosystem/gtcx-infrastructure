...
- **Target:** Named lead + link `gtcx-protocols/docs/operations/coordination/ai-reliability-owner-2026-06-06.md`
- **Implementation:** Edit AGENTS.md coordination table; update `docs/operations/coordination/README.md`; append `cross-repo-agent-log.md`
- **Dependencies:** None
...
- **Current:** `ERR_MODULE_NOT_FOUND` for `gtcx-agentic/scripts/lib/suggest-persona.mjs` @ `6092700`
- **Target:** Graceful fallback when sibling absent OR vendor `suggest-persona` into infra with re-export
- **Implementation:** Pin optional import in `scripts/agent-next-work.mjs`; document sibling checkout in `docs/agents/onboarding/orientation.md`; verify CI checkout includes gtcx-agentic
- **Dependencies:** None
...
- **Implementation:** Export gateway metrics; import baseline cost-stats; `infra/monitoring/dashboards/llm-ops.json`
...
- **Implementation:** `docs/operations/agent-pr-checklist.md` + `.github/pull_request_template.md` link
...
- Weekly `workflow_dispatch` or validate-all optional gate; evidence JSON under `docs/audit/`
...
| 4   | Per-agent prompt version | Semver `docs/audit/prompts/` tags                |
...
| 9    | `docs/architecture/agent-topology-2026-Q3.md` | Named engines + handoff diagram                                  |
...
node tools/scripts/validate-all.mjs     # ≥50/51 pass
node tools/scripts/validate-signal.mjs  # ≥9.0
```
